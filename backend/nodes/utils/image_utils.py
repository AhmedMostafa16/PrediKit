...
def normalize_normals(
    ...
    length_squared = np.square(x) + np.square(y)

    # If the length of X and Y is >1, then make it 1
    length = np.sqrt(np.maximum(length_squared, 1))
    x /= length
    y /= length
    length_squared = np.minimum(length_squared, 1, out=length_squared)

    # Compute Z
    z = np.sqrt(1 - length_squared)
...
def shift(
    img: np.ndarray, amount_x: int, amount_y: int, fill: int
) -> np.ndarray:
    c = get_h_w_c(img)[2]
    ...
    h, w, _ = get_h_w_c(img)
    translation_matrix = np.ndarray([[1, 0, amount_x], [0, 1, amount_y]])  # type: ignore
    img = cv2.warpAffine(
        img,
        translation_matrix,
        (w, h),
...
def create_border(
    ...
    if c == 4 and border_type == BorderType.BLACK:
        value: cv2.typing.Scalar = (0, 0, 0, 1)
    else:
        value = (0, 0, 0)
...
def blend_images(overlay: np.ndarray, base: np.ndarray, blend_mode: int):
    ...
    if (
        ...
        raise AssertionError("The overlay and the base image must have the same size")

    def assert_sane(c: int, name: str):
        sane = c in (1, 3, 4)
        if not sane:
            raise AssertionError(f"The {name} has to be a grayscale, RGB, or RGBA image")
    ...
    blend_rgb = blender.apply_blend(o_rgb, b_rgb, blend_mode=blend_mode)
    ...
    result = np.concatenate(
        [final_rgb, np.expand_dims(final_a, axis=2)], axis=2
    )
    if needs_clipping:
        result = np.clip(result, 0, 1)
    return result
...
def calculate_ssim(img1: np.ndarray, img2: np.ndarray) -> float:
    ...
    sigma12 = (
        cv2.filter2D(img1 * img2, -1, kernel=window)[5:-5, 5:-5] - mu1_mu2
    )
