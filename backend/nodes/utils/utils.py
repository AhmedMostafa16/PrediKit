...
def denorm(x, min_max=(-1.0, 1.0)):
    ...
    if isinstance(x, Tensor):
        ...
        raise TypeError(
            "Got unexpected object type, expected Tensor or np.ndarray"
        )
    ...
    if c == 4 and input_channels in (1, 3):
        ...
        if len(unique) == 1:
            ...
            if input_channels == 1:
                ...
                img = np.expand_dims(
                    cv2.cvtColor(img, cv2.COLOR_BGRA2GRAY), axis=2
                )
            else:
                img = img[:, :, :3]
            output = upscale(img)
            ...
            if input_channels == 1:
                logger.warning("Converting image to grayscale.")
                img1 = np.expand_dims(
                    cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY), axis=2
                )
                img2 = np.expand_dims(
                    cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY), axis=2
                )
        ...
        if c == 1:
            ...
            if input_channels == 4:
                img = np.dstack(
                    (img, np.full(img.shape[:-1], 1.0, np.float32))
                )
        # Remove extra channels if too many (i.e three channel image, single channel model)
            ...
            if input_channels == 1:
                logger.warning("Converting image to grayscale.")
                img = np.expand_dims(
                    cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), axis=2
                )
