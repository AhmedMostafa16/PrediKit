...
class ImageAverage:
    """
    A class for calculating the average of multiple images with an alpha channel.

    Attributes:
        total (np.ndarray): The accumulated sum of the images.
        count (int): The number of images added.

    Methods:
        add(img: np.ndarray): Adds an image to the total sum.
        get_result() -> np.ndarray: Calculates and returns the average image.
    ...
    def add(self, img: np.ndarray):
        """
        Adds an image to the total sum.
    ...
    def get_result(self) -> np.ndarray:
        """
        Calculates and returns the average image.
...
def fill_alpha_edge_extend(img: np.ndarray, distance: int) -> np.ndarray:
    ...
    while proccessed_distance < distance:
        ...
        k = np.zeros(
            (offset_distance * 2 + 1, offset_distance * 2 + 1),
            dtype=np.float32,
        )
        k[:, offset_distance] = 1
        k[offset_distance, :] = 1
