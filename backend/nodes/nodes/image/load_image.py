...
@NodeFactory.register("predikit:image:load")
class ImageReadNode(NodeBase):
    def __init__(self):
        super().__init__()
    ...
    def read_cv(path: str) -> np.ndarray:
        ...
        try:
            ...
            logger.warning(
                f"Error loading image, trying with imdecode: {cv_err}"
            )
    ...
    def run(self, path: str) -> Tuple[np.ndarray, str, str]:
        ...
        if not supported_by_cv and not supported_by_pil:
            raise NotImplementedError(
                f'The image "{path}" you are trying to read cannot be read by predikit.'
            )
        ...
        if img is None:
            if error is not None:
                raise error
            raise RuntimeError(
                f'Internal error loading image at path "{path}".'
            )
