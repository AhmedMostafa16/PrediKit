...
class Executor:
            ...
            for idx, node_input in enumerate(inputs):
                ...
                if isinstance(node_inputs[idx], dict):
                    ...
                    enforced_inputs.append(
                        node_inputs[idx].enforce_(node_input)
                    )
        ...
        if node["nodeType"] == "iterator":
            ...
            for v in sub_nodes.copy().values():
                ...
                for node_input in v["inputs"]:
                    ...
                    if isinstance(node_input, dict) and node_input.get(
                        "id", None
                    ):
                        next_node_id = str(node_input["id"])
                        ...
                        if next_node_id not in sub_nodes_ids:
                            logger.debug(
                                f"not in sub_node_ids, caching {next_node_id}"
                            )
    ...
    def should_stop_running(self):
        ...
        return (
            ...
            or self.paused
            or (
                self.parent_executor is not None
                and self.parent_executor.is_killed()
            )
            or (
                self.parent_executor is not None
                and self.parent_executor.is_paused()
            )
        )
