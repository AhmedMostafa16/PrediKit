...
def get_all_columns():
    try:
        ...
        df = pandas.read_json(
            orient='records', path_or_buf=StringIO(decoded_data)) if decoded_data else pandas.DataFrame()
...
def execute_node():
    try:
        ...
        df = pandas.read_json(
            orient='records', path_or_buf=StringIO(decoded_data["DataFrame"])) if decoded_data["DataFrame"] else pandas.DataFrame()

        # Process the data (you can modify this part based on your requirements)
        result = executor.execute_node(
            props=props,
            df=df,
            node_type=decoded_data["Type"]
        )
        result = result.to_json(
            orient='records') if result is not None else u''
        # Encode the result in MessagePack format
        encoded_result = msgpack.packb(result)
