...
def split_train_test(data: h2o.H2OFrame, ratios: list[int] =[0.8], seed: int = None) -> Tuple[h2o.H2OFrame, h2o.H2OFrame]:
...
class AutoML:
    def __init__(self, 
        ...
        self.model = H2OAutoML(max_runtime_secs=max_runtime_secs,
    ...
    def user_feedback(self) -> str:
        ...
        if self.model_type == 'classifier':
            with h2o.utils.threading.local_context(polars_enabled=True, datatable_enabled=True):
                pred_train_frame = LabelEncoder().fit_transform(predictions_train["predict"].as_data_frame().to_numpy().reshape(-1,))
                train_frame = LabelEncoder().fit_transform(self.train_frame[self.target].as_data_frame().to_numpy().reshape(-1,))
                pred_test_frame = LabelEncoder().fit_transform(predictions_test["predict"].as_data_frame().to_numpy().reshape(-1,))
                test_frame = LabelEncoder().fit_transform(self.test_frame[self.target].as_data_frame().to_numpy().reshape(-1,))
            if len(np.unique(test_frame)) > 2:
                pred_test_frame = (pred_test_frame - pred_test_frame.min()) / (pred_test_frame.max() - pred_test_frame.min())
                test_frame = (test_frame - test_frame.min()) / (test_frame.max() - test_frame.min())
                pred_train_frame = (pred_train_frame - pred_train_frame.min()) / (pred_train_frame.max() - pred_train_frame.min())
                train_frame = (train_frame - train_frame.min()) / (train_frame.max() - train_frame.min())
            feedback = self.classification_feedback(test_frame, pred_test_frame, train_frame, pred_train_frame)
        else:
            with h2o.utils.threading.local_context(polars_enabled=True, datatable_enabled=True):
                pred_train_frame = predictions_train["predict"].as_data_frame().to_numpy().reshape(-1,)
                train_frame = self.train_frame[self.target].as_data_frame().to_numpy().reshape(-1,)
                pred_test_frame = predictions_test["predict"].as_data_frame().to_numpy().reshape(-1,)
                test_frame = self.test_frame[self.target].as_data_frame().to_numpy().reshape(-1,)
            feedback = self.regression_feedback(test_frame, pred_test_frame, train_frame, pred_train_frame, self.train_cols_length)
        return feedback

    def get_best_model(self):
        return self.best_model

    def model_performance(self, test_data: h2o.H2OFrame,):
    ...
    def confusion_matrix(self):
        ...
        with h2o.utils.threading.local_context(polars_enabled=True, datatable_enabled=True):
            pred_test_frame = predictions_test["predict"].as_data_frame().to_numpy().reshape(-1,)
            test_frame = self.test_frame[self.target].as_data_frame().to_numpy().reshape(-1,)
        return ConfusionMatrixDisplay(confusion_matrix(test_frame, pred_test_frame)).plot()
    ...
    def classification_feedback(self, 
                                y_true_test: np.array, 
                                y_pred_test: np.array, 
                                y_true_train: np.array, 
                                y_pred_train: np.array) -> str:
        ...
        train_vs_test_accuracy = (abs(acc_train_score - acc_test_score)/(acc_train_score))
        ...
        if recall_score >= 0.75:
            feedback += f'High model recall (Recall: {(recall_score * 100):.3f}%)\n'
        ...
        if f1score >= 0.75:
            feedback += f'High model F1 Score (F1 Score: {(f1score * 100):.3f}%)\n'
        elif f1score >= 0.5:
            feedback += f'Moderate model F1 Score (F1 Score: {(f1score * 100):.3f}%)\n'
        else:
        ...
        if len(np.unique(y_true_test)) <= 2:
            if logloss_score < 0.5:
                feedback += f'Good model Log Loss (LogLoss: {(logloss_score):.3f}%)\n'
            else: 
    ...
    def regression_feedback(self, 
                            y_true_test: np.array, 
                            y_pred_test: np.array, 
                            y_true_train: np.array, 
                            y_pred_train: np.array, 
                            num_of_training_columns: int) -> str:
        ...
        adjusted_r2 = 1 - (1 - r2_score(y_true_test, y_pred_test)) * ( len(y_true_test) - 1 ) / ( len(y_true_test) - (num_of_training_columns) - 1 )

        feedback = '' 
        if r2_test >= 0.8:
            feedback += f'Good R2 score (R2 score: {(r2_test * 100):.3f}%)\n'
        elif r2_test >= 0.5:
            feedback += f'Moderate R2 score (R2 score: {(r2_test * 100):.3f}%)\n'
        else:
