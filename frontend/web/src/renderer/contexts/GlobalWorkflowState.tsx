...
interface Global {
    ...
    currentWorkflow: WorkflowDto | undefined;
...
export const GlobalProvider = memo(
    ({ children, reactFlowWrapper }: React.PropsWithChildren<GlobalProviderProps>) => {
        ...
        const [currentWorkflowId, setCurrentWorkflowId] = useSessionStorage<string>(
            "workflow-id",
            ""
        );
        ...
        const dumpState = useCallback(async (): Promise<Workflow> => {
            return {
                title: currentWorkflow?.title ?? "Untitled Workflow",
                id: getCurrentWorkflowId(),
                nodes: getNodes(),
                edges: getEdges(),
                viewport: getViewport(),
                migration: currentWorkflow?.migration ?? 0,
                version: currentWorkflow?.version ?? (await ipcRenderer.invoke("get-app-version")),
                createdAt: currentWorkflow?.createdAt ?? new Date().getTime(),
            ...
            async (savedData: ParsedSaveData, path: string, loadPosition = false) => {
                ...
                if (savedData.tamperedWith) {
                    sendAlert({
                        ...
                        message:
                            "The file you are trying to open has been modified outside of PrediKit. The modifications may cause PrediKit to behave incorrectly or in unexpected ways. The file will now be loaded with the modifications.",
                    });
                }
                ...
                if (deprecatedNodes.length > 0) {
                    ...
                    sendAlert({
                        ...
                        message: `This file contains the following deprecated node(s):\n\n${list}\n\nThis chain will still work right now, but these nodes will stop working in future versions of PrediKit.`,
                    });
                }
                ...
                setCurrentWorkflowId(savedData.id);
                setCurrentWorkflow({
                    ...savedData,
                    nodes: validNodes,
                    edges: validEdges,
        ...
        const setStateFromCloud = useCallback(
            async (id: string) => {
                if (hasRelevantUnsavedChanges) {
                    const resp = await showAlert(unsavedChangesWarning);
                    if (resp === 1) {
                ...
                //         message: `Failed to load workflow with id ${id}`,
                //     });
                //     return;
                // }
                ...
                if (savedData.nodes.length !== validNodes.length) {
                    sendAlert({
                        type: AlertType.WARN,
                        title: "Workflow contains invalid nodes",
                        message:
                            "This workflow you are trying to open contains nodes that are unavailable anymore. Check PrediKit's documentation to find deprecated. The workflow will now be loaded without the incompatible nodes.",
                    });
                }
        ...
        const setStateFromCloudRef = useRef(setStateFromCloud);
        setStateFromCloudRef.current = setStateFromCloud;
        ...
        const getAllWorkflows = async () => {
            try {
                ...
                //     // });
                //     log.error("Failed to get workflows: " + response.error);
                // }
                return workflows;
            } catch (error) {
                log.error(error);
