...
import IVLTSParser from "./antlr4/IVLTSParser";
import {
    Definition,
    Expression,
...
type ContextNames = FilterContextName<keyof typeof IVLTSParser>;
type Contexts = { [K in ContextNames]: InstanceType<(typeof IVLTSParser)[K]> };
...
class AstConverter {
    ...
    private toExpressionWithoutSource(
        ...
        if (
            context instanceof IVLTSParser.ExpressionDocumentContext ||
            context instanceof IVLTSParser.ScopeExpressionContext
        ) {
        ...
        if (context instanceof IVLTSParser.MultiplicativeExpressionContext) {
        ...
        if (context instanceof IVLTSParser.NegateExpressionContext) {
        ...
        if (context instanceof IVLTSParser.FieldAccessExpressionContext) {
        ...
        if (context instanceof IVLTSParser.PrimaryExpressionContext) {
        ...
        if (context instanceof IVLTSParser.FunctionCallContext) {
            const name = this.getName(getRequired(context, "name"));
            return new FunctionCallExpression(
                name,
        ...
        if (context instanceof IVLTSParser.MatchExpressionContext) {
    ...
    private toDefinitionsWithoutSource(
        ...
        if (context instanceof IVLTSParser.DefinitionDocumentContext) {
            return getMultiple(context, "definition").flatMap(this.toDefinitions);
        }
        if (context instanceof IVLTSParser.DefinitionContext) {
            const rule =
        ...
        if (context instanceof IVLTSParser.FunctionDefinitionContext) {
            const name = this.getName(getRequired(context, "name"));
        ...
        if (context instanceof IVLTSParser.VariableDefinitionContext) {
            const name = this.getName(getRequired(context, "name"));
            const value = this.toExpression(getRequired(context, "expression"));
            return [new VariableDefinition(name, value)];
        }
        if (context instanceof IVLTSParser.EnumDefinitionContext) {
            const name = this.getName(getRequired(context, "name"));
...
const getParser = (code: string): IVLTSParser => {
    const chars = new antlr4.InputStream(code);
    const lexer = new IVLTSLexer(chars);
    lexer.removeErrorListeners();
    lexer.addErrorListener(errorListener);
    const tokens = new antlr4.CommonTokenStream(lexer);
    const parser = new IVLTSParser(tokens);
